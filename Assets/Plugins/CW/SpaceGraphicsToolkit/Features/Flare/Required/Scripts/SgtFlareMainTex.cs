...
namespace SpaceGraphicsToolkit.Flare
{
	...
	public class SgtFlareMainTex : MonoBehaviour
	{
		...
		private void WritePixel(float u, int x)
		{
			...
			color.r *= 1.0f - SgtEase.Evaluate(ease, CwHelper.Sharpness(u, sharpnessR));
			color.g *= 1.0f - SgtEase.Evaluate(ease, CwHelper.Sharpness(u, sharpnessG));
			color.b *= 1.0f - SgtEase.Evaluate(ease, CwHelper.Sharpness(u, sharpnessB));
